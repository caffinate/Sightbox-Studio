...
| `split` | `clip`, `at` | `{"clip": id_of_second_half, "sibling": id_or_null}` — `at` is a source time strictly inside the clip; the first half keeps its id and link, `out` becomes `at`; the second half gets a new id, `in=at`, `out=`original out, `start = first_half.start + (at - first_half.in)` (immediately contiguous on the same track); if linked, the sibling splits at the same source time the same way, and the **two second-halves** get a fresh shared link (the two first-halves keep the original link) — pinned down exactly, not left ambiguous. No overlap check is needed: a clip always exclusively owns its `[start, end)` span, and the two halves exactly partition that same span with no gap or extension past either edge, so a same-track collision is provably impossible, not just unlikely |
